...
@interface FriendLineController ()<FridenLineCellDelegate,ChatKeyBoardDelegate,ChatKeyBoardDataSource>
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    float h = [FriendLineCell hyb_heightForTableView:tableView config:^(UITableViewCell *sourceCell) {
        ...
        [self configureCell:cell aatIndexPath:indexPath];
...
- (void)configureCell:(FriendLineCell *)cell aatIndexPath:(NSIndexPath *)indexPath {
...
- (void)didClickCommentBtnWithIndexPath:(NSIndexPath *)indexPath {
    ...
    FriendLineCellModel *model = self.dataArray[indexPath.row];
    self.chatKeyBoard.placeHolder = [NSString stringWithFormat:@"评论：%@",model.userName];
...
- (void)keyboardWillChangeNotification:(NSNotification *)notification {
    ...
    CGRect rect = [dict[UIKeyboardIsLocalUserInfoKey] CGRectValue];
